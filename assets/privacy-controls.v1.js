(function (window, document) {
  ...
  // Controls-only release. No Meta SDK, noscript pixel, or advertising requests.
  // Activating advertising requires a separate reviewed implementation and policy rollout.
  ...
  function savedPreference() {
    ...
    try { local = window.localStorage.getItem(KEY) === '1'; } catch (e) {}
    return { cookie: cookie, local: local };
  }
  ...
  function canLoadAdvertising() {
    // Call this at every future advertising load/event boundary, not just page init.
    return ADVERTISING_ENABLED && !getState().optedOut;
  }
  ...
  function render() {
    ...
    var state = getState();
    var message = 'No advertising opt-out is saved in this browser yet. Advertising tracking is currently off for everyone.';
    if (state.gpc) {
